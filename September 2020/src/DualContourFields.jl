# module DualContourFields

using RegionTrees
# using ConstructiveSolidGeometry: Surface, distance, normal, unitize
using Optim
...
function find_surface_intersect(v1, v2, contour::T) where {T<:Surface}
    ...
    line_func = (α)->(distance(contour, edge_func(α))^2)
    ...
    if !isempty(cell.data.residual) && !isnan(cell.data.residual)
        ...
        if isempty(cell.data.residual)
            ...
            for vertex in vertices(cell)
                if abs(distance(refinery.surface_def,vertex)) <= max_width/sqrt(2)
...
function refine_data(refinery::DualContourRefinery, boundary::HyperRectangle)
    ...
    val_array = map((x)->distance(refinery.surface_def, x), vert_array)#for now, 2D
    sign_set = Set(sign.(val_array))
    is_on_plane = length(val_array)-count(i->i==0,val_array)>1 && -1.0 in sign_set
    ...
    for v_idx = 1:length(vert_array)
        ...
        if sign_change || (current_sign_zero && next_sign_zero)
            ...
            n_local = normal(refinery.surface_def, p_local) #for now, 2D
            push!(p,p_local)
    ...
    if length(p)==1 #may be useful for debugging where flat surfaces are not involved
        ...
        try
            qef_min = simA\b
            residual = distance(refinery.surface_def, qef_min)
        catch
            qef_min = NaN
            residual = NaN
        end
...
struct DualContourField{C<:Cell} <: Surface
    root::C
end

function DualContourField(surface_def::Surface, origin::AbstractArray,
